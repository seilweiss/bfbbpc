...
void xQuickCullInit(xQCControl* ctrl, F32 xmin, F32 ymin, F32 zmin, F32 xmax, F32 ymax, F32 zmax) NONMATCH("https://decomp.me/scratch/oJ1gR")
{
...
{
    xQuickCullInit(ctrl, box->lower.x, box->lower.y, box->lower.z, box->upper.x, box->upper.y, box->upper.z);
}

S32 xQuickCullIsects(const xQCData* a, const xQCData* b)
{
    S32 result = (a->xmin <= b->xmax &&
                  a->ymin <= b->ymax &&
                  a->zmin <= b->zmax &&
                  b->xmin <= a->xmax &&
                  b->ymin <= a->ymax &&
...
{
    c->xmin = xclamp((int)((v->x - ctrl->center_x) * ctrl->scale_x), -127, 127);
    c->ymin = xclamp((int)((v->y - ctrl->center_y) * ctrl->scale_y), -127, 127);
    c->zmin = xclamp((int)((v->z - ctrl->center_z) * ctrl->scale_z), -127, 127);
    c->zmin_dup = c->zmin;
    c->xmax = c->xmin;
    c->ymax = c->ymin;
    c->zmax = c->zmin;
...
void xQuickCullForSphere(xQCControl* ctrl, xQCData* q, const xSphere* s) NONMATCH("https://decomp.me/scratch/0Yusz")
{
    xVec3 v1;
    xVec3 v2;
