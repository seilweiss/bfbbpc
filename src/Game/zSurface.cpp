...
static S32 zSurfaceEventCB(xBase* from, xBase* to, U32 toEvent, const F32* toParam, xBase* toParamWidget);

void zSurfaceInit() NONMATCH("https://decomp.me/scratch/TWW7Z")
{
    U32 size;
    U16 nsurfs;
    ...
    if (nsurfs) {
        zsps = (zSurfaceProps*)xMALLOC(nsurfs * sizeof(zSurfaceProps));
        ...
        for (U16 i = 0; i < nsurfs; i++) {
            ...
            for (S32 j = 0; j < 2; j++) {
                ...
                if (moprops->uvfx[j].rot <= 360.0f) moprops->uvfx[j].rot = 360.0f;
                if (moprops->uvfx[j].rot >= 0.0f) moprops->uvfx[j].rot = 0.0f;
...
static void zSurfaceInitDefaultSurface() NONMATCH("https://decomp.me/scratch/cjOeP")
{
...
void zSurfaceRegisterMapper(U32 assetId) NONMATCH("https://decomp.me/scratch/LW5tF")
{
...
{
    if (surf->moprops) {
        return ((zSurfaceProps*)surf->moprops)->asset->phys_flags & 0x1;
    }
...
{
    if (surf->moprops) {
        return ((zSurfaceProps*)surf->moprops)->asset->phys_flags & 0x4;
    }
...
{
    if (s.moprops) {
        return ((zSurfaceProps*)s.moprops)->asset->phys_flags & 0x10;
    }
...
{
    if (surf->moprops) {
        return xdeg2rad(((zSurfaceProps*)surf->moprops)->asset->sld_start);
    }
...
{
    if (surf->moprops) {
        return xdeg2rad(((zSurfaceProps*)surf->moprops)->asset->sld_stop);
    }
...
{
    if (surf->moprops) {
        return ((zSurfaceProps*)surf->moprops)->asset->phys_flags & 0x2;
    }
...
{
    if (surf->moprops) {
        return ((zSurfaceProps*)surf->moprops)->asset->game_damage_flags & 0x1;
    }
...
{
    if (surf->moprops) {
        return !(((zSurfaceProps*)surf->moprops)->asset->phys_flags & 0x8);
    }
...
S32 zSurfaceGetSlickness(const xSurface* surf) NONMATCH("https://decomp.me/scratch/UkEyn")
{
...
void zSurfaceUpdate(xBase* to, xScene* sc, F32 dt) NONMATCH("https://decomp.me/scratch/1rmMO")
{
    ...
    for (j = 0; j < 2; j++) {
        if ((j == 0 && (moprops->uvfx_flags & UVANIM_FLAG_ON)) ||
            ...
            case 0:
            {
                ...
                while (moprops->uvfx[j].rot >= 360.0f) moprops->uvfx[j].rot -= 360.0f;
                while (moprops->uvfx[j].rot <  0.0f)   moprops->uvfx[j].rot += 360.0f;
                
                xVec3SMul(&d, &moprops->uvfx[j].trans_spd, timestep_forUV);
                xVec3Add(&moprops->uvfx[j].trans, &d, &moprops->uvfx[j].trans);
                while (moprops->uvfx[j].trans.x >= 1.0f) moprops->uvfx[j].trans.x -= 1.0f;
                while (moprops->uvfx[j].trans.x <  0.0f) moprops->uvfx[j].trans.x += 1.0f;
                while (moprops->uvfx[j].trans.y >= 1.0f) moprops->uvfx[j].trans.y -= 1.0f;
                while (moprops->uvfx[j].trans.y <  0.0f) moprops->uvfx[j].trans.y += 1.0f;

                xVec3SMul(&d, &moprops->uvfx[j].scale_spd, dt);
                xVec3Add(&moprops->uvfx[j].scale, &d, &moprops->uvfx[j].scale);
                while (moprops->uvfx[j].scale.x >= 8.0f) moprops->uvfx[j].scale.x -= 8.0f;
                while (moprops->uvfx[j].scale.x <  0.0f) moprops->uvfx[j].scale.x += 8.0f;
                while (moprops->uvfx[j].scale.y >= 8.0f) moprops->uvfx[j].scale.y -= 8.0f;
                while (moprops->uvfx[j].scale.y <  0.0f) moprops->uvfx[j].scale.y += 8.0f;

                break;
            }
            case 1:
            {
                moprops->uvfx[j].trans.x = isin(2.0f * gFrameCount * SECS_PER_VBLANK);
                moprops->uvfx[j].trans.y = isin(2.0f * gFrameCount * SECS_PER_VBLANK);
                moprops->uvfx[j].scale.x = isin(2.0f * gFrameCount * SECS_PER_VBLANK);
                moprops->uvfx[j].scale.y = isin(2.0f * gFrameCount * SECS_PER_VBLANK);
                break;
            }
            case 2:
            {
                ...
                if (sfx.minmax_spd.x != 0.0f) {
                    ...
                    sfx.trans.x = sfx.min.x + (sfx.max.x - sfx.min.x) * isin(PI/2 * uTime);
                } else {
                ...
                if (sfx.minmax_spd.y != 0.0f) {
                    ...
                    sfx.trans.y = sfx.min.y + (sfx.max.y - sfx.min.y) * isin(PI/2 * vTime);
                } else {
        ...
        if ((j == 0 && (moprops->texanim_flags & SURF_TEXANIM_ON)) ||
            ...
            if (g) {
                ...
                if (max > 0) {
                    ...
                    while (moprops->texanim[j].group_idx >= max) moprops->texanim[j].group_idx -= max;
                    while (moprops->texanim[j].group_idx < 0) moprops->texanim[j].group_idx += max;
                }
            }
        }
    }
...
static S32 zSurfaceEventCB(xBase* from, xBase* to, U32 toEvent, const F32* toParam, xBase* toParamWidget) NONMATCH("https://decomp.me/scratch/Mgcgy")
{
    xSurface* t = (xSurface*)to;
...
void zSurfaceGetName(S32 type, char* buffer)
{
    *buffer = '\0';
    ...
    switch (type) {
    case 0:  strcpy(buffer, "DEFAULT");      break;
    case 1:  strcpy(buffer, "TILE");         break;
    case 2:  strcpy(buffer, "CARPET");       break;
    case 3:  strcpy(buffer, "SHORTGRASS");   break;
    case 4:  strcpy(buffer, "LONGGRASS");    break;
    case 5:  strcpy(buffer, "GRAVEL");       break;
    case 6:  strcpy(buffer, "DIRT");         break;
    case 7:  strcpy(buffer, "MUD");          break;
    case 8:  strcpy(buffer, "THORNS");       break;
    case 9:  strcpy(buffer, "METAL");        break;
    case 10: strcpy(buffer, "SHEETMETAL");   break;
    case 11: strcpy(buffer, "CONCRETE");     break;
    case 12: strcpy(buffer, "MARBLE");       break;
    case 13: strcpy(buffer, "STONE");        break;
    case 14: strcpy(buffer, "WOOD");         break;
    case 15: strcpy(buffer, "SNOW");         break;
    case 16: strcpy(buffer, "ICE");          break;
    case 17: strcpy(buffer, "SHALLOWWATER"); break;
    case 18: strcpy(buffer, "DEEPWATER");    break;
    case 19: strcpy(buffer, "TAR");          break;
    case 20: strcpy(buffer, "SPECIAL1");     break;
    case 21: strcpy(buffer, "DeepAcid");     break;
    case 22: strcpy(buffer, "NONE");         break;
    }
}
