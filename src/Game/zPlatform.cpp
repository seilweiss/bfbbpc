...
static zParEmitter* sEmitTremble;
static zParEmitter* sEmitBreakaway;
...
void zPlatform_Init(zPlatform* plat, xEntAsset* asset) NONMATCH("https://decomp.me/scratch/w4O9T")
{
    ...
    if (plat->subType == ePlatformTypeBreakaway) {
        plat->collis = (xEntCollis*)xMALLOC(sizeof(xEntCollis));
        plat->collis->chk = 0;
        plat->collis->pen = 0;
        ...
        plat->fmrt = (zPlatFMRunTime*)xMALLOC(sizeof(zPlatFMRunTime));
    }
...
void zPlatform_Reset(zPlatform* plat, xScene* sc) NONMATCH("https://decomp.me/scratch/89lwW")
{
    if (plat->subType == ePlatformTypeBreakaway) {
        plat->model = plat->am;
        plat->collModel = NULL;
    ...
    if (plat->subType == ePlatformTypeER) {
        ...
        plat->state = 0;
        plat->pflags &= (U8)~(k_XENT_HAS_VELOCITY | k_XENT_HAS_GRAVITY);
        plat->collis->chk = 0;
    ...
    plat->chkby &= (U8)~(k_XENT_COLLTYPE_PC | k_XENT_COLLTYPE_NPC | k_XENT_COLLTYPE_DYN);

    if (plat->passet->flags & 0x4) {
        plat->chkby |= (k_XENT_COLLTYPE_PC | k_XENT_COLLTYPE_NPC);
    }
...
static F32 SolvePaddleMotion(zPlatform* plat, F32* time, F32 tmr) NONMATCH("https://decomp.me/scratch/I3xDP")
{
...
U32 zPlatform_PaddleCollide(xCollis* coll, const xVec3* hitsource, const xVec3* hitvel, U32 worldSpaceNorm) NONMATCH("https://decomp.me/scratch/Vv33p")
{
...
static void zPlatFM_Update(zPlatform* plat, xScene*, F32 dt) NONMATCH("https://decomp.me/scratch/GlT4R")
{
    for (S32 i = 0; i < 12; i++) {
        ...
        if (tm > 0.0f) {
            ...
            fmrt->tmrs[i] -= dt;
            if (fmrt->tmrs[i] < 0.0f) {
                F32 rts = xsqrt(xsqr(pmat->right.x) + xsqr(pmat->right.y) + xsqr(pmat->right.z));
                F32 ups = xsqrt(xsqr(pmat->up.x) + xsqr(pmat->up.y) + xsqr(pmat->up.z));
                F32 ats = xsqrt(xsqr(pmat->at.x) + xsqr(pmat->at.y) + xsqr(pmat->at.z));
...
void zPlatform_Update(xEnt* entplat, xScene* sc, F32 dt)
{
    ...
    if (plat->subType != ePlatformTypeBreakaway) {
        plat->pauseMult = xclamp(plat->pauseMult + plat->pauseDelta, 0.000001f, 1.0f);
        dt *= plat->pauseMult;
    }
    ...
    if (plat->subType == ePlatformTypeER) {
        ...
        if (plat != drv->odriver && plat != drv->driver) {
            ...
            F32 dtilt = xabs(xdeg2rad(plat->passet->teet.invmass) * dt);
            if (ctilt != itilt) {
                if (ctilt < itilt) {
                    dtilt = xmin(dtilt, itilt - ctilt);
                } else if (ctilt > itilt) {
                    dtilt = xmax(-dtilt, itilt - ctilt);
                }
            ...
            F32 dangle = -s * xdeg2rad(plat->passet->teet.invmass) * lpos.x * dt;
        ...
        if (plat->tmr) {
            ...
            F32 time[3];
            F32 orient = xdeg2rad(SolvePaddleMotion(plat, time, plat->tmr));
            xMat3x3 orientrot;
...
void zPlatform_Shake(zPlatform* plat, F32, F32 ampl, F32 freq) NONMATCH("https://decomp.me/scratch/dZz4G")
{
    xFFXShakeState* ss = xFFXShakeAlloc();
    if (!ss) return;
...
static void zPlatform_Tremble(zPlatform* plat, F32 ampl, F32 freq, F32 dur) NONMATCH("https://decomp.me/scratch/5s6xq")
{
    xFFXShakeState* ss = xFFXShakeAlloc();
    if (ss) {
        xFFX* sfkt = xFFXAlloc();
        ...
        xVec3SMul(&ss->disp, &globals.camera.mat.right, ampl);
    ...
    if (sEmitTremble) {
        ...
        for (S32 i = 0; i < 25; i++) {
            xParEmitterEmitCustom(sEmitTremble, 1/30.f, &info);
        }
    }
}
...
void zPlatform_Mount(zPlatform* plat) NONMATCH("https://decomp.me/scratch/6YdJp")
{
...
static void zPlatFM_EventSetup(zPlatform* plat, const F32* toParam, S32 idx) NONMATCH("https://decomp.me/scratch/BYLtF")
{
    zPlatFMRunTime* fmrt = plat->fmrt;
    if (fmrt->flags & (1 << idx)) return;
    ...
    if (atm > ttm) atm = ttm;
    else if (atm < 0.0f) atm = 0.0f;
    atm = ttm - atm;

    if (dtm > atm) dtm = atm;
    else if (dtm < 0.0f) dtm = 0.0f;

    fmrt->tmrs[idx] = ttm;
    fmrt->ttms[idx] = ttm;
    fmrt->atms[idx] = atm;
    fmrt->dtms[idx] = dtm;
    fmrt->vms[idx] = 2.0f * ds / (ttm - dtm + atm);
    fmrt->dss[idx] = ds;
    fmrt->flags |= (1 << idx);
}

S32 zPlatformEventCB(xBase* from, xBase* to, U32 toEvent, const F32* toParam, xBase* toParamWidget) NONMATCH("https://decomp.me/scratch/Uj05m")
{
    ...
    case eEventRun:
        if (plat->subType == ePlatformTypeBreakaway) {
            ...
            if (plat->subType == ePlatformTypeMech &&
                globals.sceneCur->sceneID == IDTAG('P','G','1','2') &&
