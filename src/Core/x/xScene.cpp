...
void xSceneInit(xScene* sc, U16 num_trigs, U16 num_stats, U16 num_dyns, U16 num_npcs)
{
    ...
    sc->trigs = (xEnt**)xMALLOC(num_trigs * sizeof(xEnt*));
    sc->num_stats_allocd = num_stats;
    sc->stats = (xEnt**)xMALLOC(num_stats * sizeof(xEnt*));
    sc->num_dyns_allocd = num_dyns;
    sc->dyns = (xEnt**)xMALLOC(num_dyns * sizeof(xEnt*));
    sc->num_npcs_allocd = num_npcs;
    sc->npcs = (xEnt**)xMALLOC(num_npcs * sizeof(xEnt*));
    sc->num_ents_allocd = (U32)num_trigs + (U32)num_stats + (U32)num_dyns + (U32)num_npcs;
    sc->num_act_ents = 0;
    sc->act_ents = (xEnt**)xMALLOC(sc->num_ents_allocd * sizeof(xEnt*));
    sc->num_nact_ents = 0;
    sc->nact_ents = (xEnt**)xMALLOC(sc->num_ents_allocd * sizeof(xEnt*));
    sc->num_ents = 0;
    sc->num_trigs = 0;
    sc->num_stats = 0;
    sc->num_dyns = 0;
...
void xSceneForAllEnts(xScene* sc, xEntCallback func, void* data)
{
    for (U16 i = 0; i < sc->num_act_ents; i++) {
        if (!func(sc->act_ents[i], sc, data)) break;
    }
}
...
void xSceneForAllStatics(xScene* sc, xEntCallback func, void* data)
{
    for (U16 i = 0; i < sc->num_stats; i++) {
        if (!func(sc->stats[i], sc, data)) break;
    }
}
...
void xSceneForAllDynamics(xScene* sc, xEntCallback func, void* data)
{
    for (U16 i = 0; i < sc->num_dyns; i++) {
        if (!func(sc->dyns[i], sc, data)) break;
    }
}
...
void xSceneForAllNPCs(xScene* sc, xEntCallback func, void* data)
{
    for (U16 i = 0; i < sc->num_npcs; i++) {
        if (!func(sc->npcs[i], sc, data)) break;
    }
}

void xRayHitsGrid(xGrid* grid, xScene* sc, xRay3* r, xRayEntCallback rentcb, xQCData* qcr, void* data) NONMATCH("https://decomp.me/scratch/2kH6a")
{
    xLine3 ln;
    xGridIterator it;
    ...
    if (ln.p1.z < ln.p2.z) {
        sz = sclgz;
        ez = eclgz;
        if (sclgz > 0 && ln.p1.z < sclcz - quatrclz) sz--;
        if (eclgz < grid->nz - 1 && ln.p2.z > eclcz + quatrclz) ez++;
        for (z = sz; z <= ez; z++) {
            F32 cz;
            F32 lz, rz;
            F32 lx, rx;
            ...
            if (lx < rx) {
                ax = aclgx;
                bx = bclgx;
                if (aclgx > 0 && lx < aclcx - quatrclx) ax--;
                if (bclgx < grid->nx - 1 && rx > bclcx + quatrclx) bx++;
                ...
                ax = aclgx;
                bx = bclgx;
                if (bclgx > 0 && rx < bclcx - quatrclx) bx--;
                if (aclgx < grid->nx - 1 && lx > bclcx + quatrclx) ax++;
        ...
        sz = sclgz;
        ez = eclgz;
        if (eclgz > 0 && ln.p2.z < eclcz - quatrclz) ez--;
        if (sclgz < grid->nz - 1 && ln.p1.z > sclcz + quatrclz) sz++;
        for (z = ez; z <= sz; z++) {
            F32 cz;
            F32 lz, rz;
            F32 lx, rx;
...
void xRayHitsTikiLandableEnt(xScene* sc, xRay3* r, xQCData* qcr, xEnt* ent, void* colldata)
{
    ...
    if (ent->chkby == 0) return;
    if (ent->collType == k_XENT_COLLTYPE_NPC) return;
    if (ent->baseType == eBaseTypeBoulder) return;
    if (qcr && !xQuickCullIsects(qcr, &ent->bound.qcd)) return;

    xCollis c;
    xRayHitsBound(r, &ent->bound, &c);
    
    if (!(c.flags & k_HIT_IT)) return;
...
void xRayHitsTikiLandableScene(xScene* sc, xRay3* r, xCollis* coll) NONMATCH("https://decomp.me/scratch/sRaVC")
{
    coll->dist = HUGE;
...
void xRayHitsScene(xScene* sc, xRay3* r, xCollis* coll) NONMATCH("https://decomp.me/scratch/Q5Giy")
{
    coll->dist = HUGE;
...
struct cb_ray_hits_ent
{
    ...
    bool operator()(xEnt& ent, xGridBound&)
    {
        if (!(ent.chkby & chkby)) return true;
        if (!(ent.collType & collType)) return true;

        xCollis tempColl;
        tempColl.flags = coll.flags;
        xRayHitsBound(&ray, &ent.bound, &tempColl);

        if (!(tempColl.flags & k_HIT_IT)) return true;
        if (tempColl.dist > coll.dist) return true;

        if (ent.collLev == 5) {
            iRayHitsModel(&ray, ent.model, &tempColl);
            if (!(tempColl.flags & k_HIT_IT)) return true;
            if (tempColl.dist > coll.dist) return true;
        }

        memcpy(&coll, &tempColl, sizeof(xCollis));
        coll.oid = ent.id;
        coll.optr = &ent;
        coll.mptr = ent.model;
...
void xRayHitsSceneFlags(xScene* sc, xRay3* r, xCollis* coll, U8 collType, U8 chk) NONMATCH("https://decomp.me/scratch/ZJEEb")
{
    coll->dist = HUGE;
...
static void ProjectTriangle(xVec3* rkD, xVec3* apkTri, F32* rfMin, F32* rfMax)
{
    F32 fDot;
    
    *rfMin = rkD->x * apkTri[0].x + rkD->y * apkTri[0].y + rkD->z * apkTri[0].z;
    *rfMax = *rfMin;
    
    fDot = rkD->x * apkTri[1].x + rkD->y * apkTri[1].y + rkD->z * apkTri[1].z;
    if (fDot < *rfMin) {
        *rfMin = fDot;
    } else if (fDot > *rfMax) {
        *rfMax = fDot;
    ...
    fDot = rkD->x * apkTri[2].x + rkD->y * apkTri[2].y + rkD->z * apkTri[2].z;
    if (fDot < *rfMin) {
        *rfMin = fDot;
    } else if (fDot > *rfMax) {
        *rfMax = fDot;
...
static void ProjectBox(xVec3* rkD, xBox* rkBox, F32* rfMin, F32* rfMax)
{
    F32 fDdC = 0.5f *
        (rkD->x * (rkBox->upper.x + rkBox->lower.x) +
         rkD->y * (rkBox->upper.y + rkBox->lower.y) +
         rkD->z * (rkBox->upper.z + rkBox->lower.z));
    F32 fR = 0.5f *
        (xabs(rkD->x * (rkBox->upper.x - rkBox->lower.x)) +
         xabs(rkD->y * (rkBox->upper.y - rkBox->lower.y)) +
         xabs(rkD->z * (rkBox->upper.z - rkBox->lower.z)));

    *rfMin = fDdC - fR;
    *rfMax = fDdC + fR;
}

static U32 Mgc_TriBoxTest(xVec3* apkTri, xBox* rkBox) NONMATCH("https://decomp.me/scratch/xYaVl")
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (curflag == 0) return 1;
        
        andflag &= curflag;
    }

    if (andflag != 0) return 0;

    akE[0].x = apkTri[1].x - apkTri[0].x,
        akE[0].y = apkTri[1].y - apkTri[0].y,
        akE[0].z = apkTri[1].z - apkTri[0].z;
    akE[1].x = apkTri[2].x - apkTri[0].x,
        akE[1].y = apkTri[2].y - apkTri[0].y,
    ...
    if (fMax1 < fMin0 || fMax0 < fMin1) return 0;

    baxis.x = 0.0f;
    baxis.y = 0.0f;
    baxis.z = 0.0f;

    akE[2].x = akE[1].x - akE[0].x,
        akE[2].y = akE[1].y - akE[0].y,
        akE[2].z = akE[1].z - akE[0].z;
...
static RpCollisionTriangle* nearestFloorCB(RpIntersection*, RpCollisionTriangle* collTriangle, RwReal, void* data) NONMATCH("https://decomp.me/scratch/0VY4M")
{
    ...
    if ((f3 >= 0.0f && f2 >= 0.0f && f1 >= 0.0f) ||
        (f3 <= 0.0f && f2 <= 0.0f && f1 <= 0.0f)) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (numer <= 0.0f) {
            testdist2 = xsqr(centvec.x) + xsqr(centvec.y) + xsqr(centvec.z);
            ...
            testdist2 = xsqr(nfpoly->center.x - testpt.x) +
                        xsqr(nfpoly->center.y - testpt.y) +
                        xsqr(nfpoly->center.z - testpt.z);
    ...
    if (currnear != nfpoly->neardist) {
        if (nfpoly->facingDot > 0.0f) {
            ...
            nearmag = xsqrt(xsqr(neardx) + xsqr(neardz));
            
            if (nearmag < 0.000001f) {
                return collTriangle;
            }

            if ((neardx * nfpoly->facingVec.x + neardz * nfpoly->facingVec.z) / nearmag < nfpoly->facingDot) {
                return collTriangle;
            }
        }
...
static RpCollisionTriangle* boxNearestFloorCB(RpIntersection* intersection, RpCollisionTriangle* collTriangle, RwReal distance, void* data)
{
    xNearFloorPoly* nfpoly = (xNearFloorPoly*)data;
    xVec3 xformVert[3];
...
static RpCollisionTriangle* sectorNearestFloorCB(RpIntersection* intersection, RpWorldSector*, RpCollisionTriangle* collTriangle, RwReal distance, void* data)
{
    return nearestFloorCB(intersection, collTriangle, distance, data);
}
...
static S32 gridNearestFloorCB(xEnt* ent, void* cbdata)
{
    ...
    if (!(ent->moreFlags & 0x80)) return 1;
    if (!(ent->collType & sNearestChk)) return 1;
    if (!(ent->chkby & sNearestCollType)) return 1;
    
    xCollis tempC;
    tempC.flags = 0;

    xBoundHitsBound(&sNearestBound, &ent->bound, &tempC);
    if (!(tempC.flags & k_HIT_IT)) return 1;
...
U32 xSceneNearestFloorPoly(xScene* sc, xNearFloorPoly* nfpoly, U8 collType, U8 chk) NONMATCH("https://decomp.me/scratch/yCzQV")
{
    sNearestBound.type = k_XBOUNDTYPE_BOX;
    sNearestBound.box.box = nfpoly->box;

    xVec3Lerp(&sNearestBound.box.center, &sNearestBound.box.box.upper, &sNearestBound.box.box.lower, 0.5f);
    ...
    sSphereIsx.t.sphere.radius = xsqrt(
        (xsqr(isx.t.box.sup.x - isx.t.box.inf.x) +
         xsqr(isx.t.box.sup.y - isx.t.box.inf.y) +
         xsqr(isx.t.box.sup.z - isx.t.box.inf.z)) * 0.25f);
    ...
    if (chk & (k_XENT_COLLTYPE_STAT | k_XENT_COLLTYPE_DYN)) {
        ...
        xGridCheckPosition(&colls_grid, &sNearestBound.box.center, &sNearestBound.qcd, gridNearestFloorCB, nfpoly);
        xGridCheckPosition(&colls_oso_grid, &sNearestBound.box.center, &sNearestBound.qcd, gridNearestFloorCB, nfpoly);
    }
    ...
    if (chk & k_XENT_COLLTYPE_ENV) {
        ...
        if (sc->env->geom->jsp) {
            sUseJSP = true;
            xClumpColl_ForAllIntersections(sc->env->geom->jsp->colltree, &isx, sectorNearestFloorCB, nfpoly);
            sUseJSP = false;
        } else {
            RpCollisionWorldForAllIntersections(sc->env->geom->world, &isx, sectorNearestFloorCB, nfpoly);
        }
    }
